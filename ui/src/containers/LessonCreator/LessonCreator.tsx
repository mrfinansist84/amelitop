...
export const LessonCreator: React.FC = () => {
  ...
  const availableElementsList = () => {
    return addingBtn.map((item: any) => (
      <div className="side-nav__element d-flex align-items-center justify-content-center" key={item.id} onClick={() => addElement(item.type)}>
        <h3 className="side-nav__element-title">{item.title}</h3>
      </div>
    ));
  };

  const changeOrder = (filteredList: any) => filteredList.forEach((item: any, index: number) => {
    item.index = index + 1;
  });
